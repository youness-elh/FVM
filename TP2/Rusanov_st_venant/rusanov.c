...
void fnum(double *wL, double *wR, double *flux){
    ...
    double lambdaL = fabs(compute_vmax(wL));
    double lambdaR = fabs(compute_vmax(wR));
    double lambda = lambdaR > lambdaL ? lambdaR : lambdaL;

    for(int iv = 0; iv < _M; iv++){
      flux[iv] = 0.5 * (fL[iv] + fR[iv]) - 0.5 * lambda * (wR[iv] - wL[iv]);
    }
